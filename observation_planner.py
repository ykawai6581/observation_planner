...
with requests.Session() as s:
    ...
    for i, plan in enumerate(plans):
        ...
        plan = plan.sort_values(['Filler','Priority','Name'],ascending=[False,False,False]).reset_index()
        ...
        for index, object in plan.iterrows():
            ...
            twilight_filter = (df_altitude_plot['UT'] > night_twilight) & (df_altitude_plot['UT'] < morning_twilight)
            ...
            object_df_list.append([df_altitude_plot[['Alt','Az']],object['Name'],obs_lim_filter,intransit,ootransit,object,meta,df_altitude_plot])

            #このループでプロットまでしているけれど、本来的にはここでループを終わらせて、計算した後に求めた天体だけでプロットしたい（もしくは、観測する天体以外をグレースケールにする）
            print(index)
        
        #ここからパラテンできるね！
        #必要になっている変数は、planとconstants、observation matrix, transit matrix
        count = 0
        steps = 700_000
        burn  = 100_000
        thin = 200
        ...
        while count <= steps:
            ...
            for index in range(num_chains):
                ...
                observed_fraction_list[index].append(observed_fraction_exp)
        ...
        observed_fraction_list = np.array(observed_fraction_list)

        expected_cost = np.cumsum(cost_list[:,burn::thin],axis=1)/list(range(1,len(cost_list[:,burn::thin][0])+1))
        expected_fraction = np.cumsum(observed_fraction_list[:,burn::thin],axis=1)/list(range(1,len(observed_fraction_list[:,burn::thin][0])+1))

        #print(exchange_list)

        sorted_cost = sorted(set(cost_list[-1][burn::thin]))
        sorted_fraction = sorted(set(observed_fraction_list[-1][burn::thin]),reverse=True)

        best_indices = [list(cost_list[-1]).index(sorted_cost[i]) for i in range(min(10,len(sorted_cost),len(sorted_fraction)))] # i access the index of the original cost list without burn or thin
        best_indices_2 = [list(observed_fraction_list[-1]).index(sorted_fraction[i]) for i in range(min(10,len(sorted_cost),len(sorted_fraction)))] # i access the index of the original cost list without burn or thin
        ...
        fig_pt, ax = plt.subplots(num_chains,3,sharex='col')
        for i in range(num_chains):
            ...
            ax[i,2].plot(range(len(expected_fraction[-1])),expected_fraction[i],label=f"T = {temperatures[i]:.3f}")
            ax[i,2].legend(loc="upper right")
            fig_pt.tight_layout()
        ax[0,0].set_title("Expected cost")
        ax[0,1].set_title("Cost distribution")
        ax[0,2].set_title("Expected observed fraction")
        ...
        for index,(position,object_name,obs_lim_filter,intransit,ootransit,object,meta,df_altitude_plot) in enumerate(object_df_list):

            is_planned = np.sum(best_matrix[::-1][index]) != 0
            is_planet = type(object['Obs begin DT']) != pd._libs.tslibs.nattype.NaTType
            
            if is_planned:#その天体を観測するというプランであれば
                color = np.random.uniform(low=0.42, high=0.95, size=(3,))
                text_color = darken_color(color, 0.3)
                #ここにleftとobs_durationも指定する1が初めて登場するindexにおけるconstants['UT']の値
                
                a = np.append(False, best_matrix[::-1][index].astype(bool))
                a = np.append(a, False)
                b = ~a
                #print(a,b)
                left_index  = list(a[1:]*b[:-1]).index(True)  #こっちは左が一個減るから+1しないとカウントがずれてしまう
                right_index = list(b[1:]*a[:-1]).index(True) - 1 #こっちは右が一個減るから+1しなくてもカウントがずれていない

                left = mdates.date2num(constants['UT'][left_index])
                right = mdates.date2num(constants['UT'][right_index])
                ...
                try:
                    ...
                    pass
                print(f'Obs time: {constants["UT"][left_index].strftime("%H:%M")} - {constants["UT"][right_index].strftime("%H:%M")}')
                ...
                text_color = darken_color(color, 0.3)
                if is_planet: #その中でもfillerでなければ
                    left = mdates.date2num(object['Obs begin DT'])
                    right = mdates.date2num(object['Obs end DT'])
                else:#fillerであった場合は観測時間をobslimfilterで定義する
                    left = mdates.date2num(df_altitude_plot['UT'][obs_lim_filter].iloc[-1])
                    right = mdates.date2num(df_altitude_plot['UT'][obs_lim_filter].iloc[0])
            
            obs_duration = right - left
            colors.append(color)
            ...
            if is_planet:
                ...
                ax_gantt_plot.barh(object['Name'], left=left, width=obs_duration, color=color,alpha=0.4,height=1)#, left=df_altitude_plot['JST'])
                ...
                observation_plot, = ax_gantt_plot.barh(object['Name'], left=left, width=obs_duration, color=color,height=0.5,alpha=0.4)#, left=df_altitude_plot['JST'])
                ax_gantt_plot.text(left + obs_duration/2, object['Name'], f'{object["Name"]} [{str(object["Priority"])}|{object["Filler"]}]', va='center' ,ha='center', fontsize=10, color=text_color,weight='bold')
        ...
        def animate_planets(j):
            ...
            if today:
                ...
                now = constants['UT'].iloc[0] + datetime.timedelta(minutes=(j*5))
            if constants['UT'].iloc[0] < now and constants['UT'].iloc[-1] > now:
                ...
                if (j==0 or j%(resolution*60) == 0) or (not today):
                    ...
                    live_color = [darken_color(color,0.3) for color in colors]
